...
import (
	...
	"github.com/polynetwork/mxc-relayer/tools"
...
func NewMSCManager(servconfig *config.ServiceConfig, startheight uint64, startforceheight uint64, ontsdk *sdk.PolySdk, client *ethclient.Client, boltDB *db.BoltDB) (*MSCManager, error) {
	...
	if !common.FileExisted(servconfig.PolyConfig.WalletFile) {
		...
		if err != nil {
			log.Errorf("NewMSCManager - wallet open error: %s", err.Error())
			return nil, err
		}
	}
	...
	if err != nil || signer == nil {
		...
		if err != nil {
			log.Errorf("NewMSCManager - wallet password error")
			return nil, err
		}
	...
	log.Infof("NewMSCManager - poly address: %s", signer.Address.ToBase58())
...
func (this *MSCManager) MonitorChain() {
	fetchBlockTicker := time.NewTicker(config.MSC_MONITOR_INTERVAL)
	var (
	...
	for {
		...
		case <-fetchBlockTicker.C:
			this.height, err = tools.GetNodeHeight(this.config.MSCConfig.URL(), this.restClient)
			if err != nil {
				log.Infof("MSCManager MonitorChain - cannot get node height, err: %s", err)
				continue
			}
			if this.height-this.currentHeight <= config.MSC_USEFUL_BLOCK_NUM {
				continue
			}

			blockHandleResult = true

			for this.currentHeight < this.height-config.MSC_USEFUL_BLOCK_NUM {
				log.Infof("MSCManager MonitorChain handleNewBlock %d", this.currentHeight+1)
				...
				if len(this.header4sync) >= this.config.MSCConfig.HeadersPerBatch {
...
func (this *MSCManager) init() error {
	// get latest height
	...
	if this.forceHeight > 0 && this.forceHeight < latestHeight {
		...
		this.currentHeight = latestHeight - this.config.MSCConfig.BlockConfig
	}
	log.Infof("MSCManager init - start height: %d", this.currentHeight)
	return nil
}

func (this *MSCManager) findLastestHeight() uint64 {
	// try to get key
	var sideChainIdBytes [8]byte
	binary.LittleEndian.PutUint64(sideChainIdBytes[:], this.config.MSCConfig.SideChainId)
...
func (this *MSCManager) handleNewBlock(height uint64) bool {
...
func (this *MSCManager) handleBlockHeader(height uint64) bool {
	...
	raw, _ := this.polySdk.GetStorage(autils.HeaderSyncContractAddress.ToHexString(),
		append(append([]byte(scom.MAIN_CHAIN), autils.GetUint64Bytes(this.config.MSCConfig.SideChainId)...), autils.GetUint64Bytes(height)...))
...
func (this *MSCManager) fetchLockDepositEvents(height uint64, client *ethclient.Client) bool {
	lockAddress := ethcommon.HexToAddress(this.config.MSCConfig.ECCMContractAddress)
...
func (this *MSCManager) commitHeader() int {
	start := time.Now()
	tx, err := this.polySdk.Native.Hs.SyncBlockHeader(
		this.config.MSCConfig.SideChainId,
	...
	log.Infof("MSCManager MonitorChain - commitHeader - send transaction %s to poly chain and confirmed on height %d, synced mxc height %d, mxc height %d, took %s, header count %d", tx.ToHexString(), h, this.currentHeight, this.height, time.Now().Sub(start).String(), len(this.header4sync))
	this.header4sync = make([][]byte, 0)
	return 0
}

func (this *MSCManager) rollBackToCommAncestor() {
	for ; ; this.currentHeight-- {
		raw, err := this.polySdk.GetStorage(autils.HeaderSyncContractAddress.ToHexString(),
			append(append([]byte(scom.MAIN_CHAIN), autils.GetUint64Bytes(this.config.MSCConfig.SideChainId)...), autils.GetUint64Bytes(this.currentHeight)...))
...
func (this *MSCManager) MonitorDeposit() {
	monitorTicker := time.NewTicker(config.MSC_MONITOR_INTERVAL)
	for {
		select {
		case <-monitorTicker.C:
			height, err := tools.GetNodeHeight(this.config.MSCConfig.URL(), this.restClient)
			if err != nil {
			...
			log.Log.Info("MonitorDeposit mxc - snyced mxc height", snycheight, "mxc height", height, "diff", height-snycheight)
...
func (this *MSCManager) handleLockDepositEvents(refHeight uint64) error {
	...
	for _, v := range retryList {
		...
		if refHeight <= crosstx.height+this.config.MSCConfig.BlockConfig {
			continue
		}
		height := int64(refHeight - this.config.MSCConfig.BlockConfig)
		heightHex := hexutil.EncodeBig(big.NewInt(height))
		proofKey := hexutil.Encode(keyBytes)
		//2. get proof
		proof, err := tools.GetProof(this.config.MSCConfig.URL(), this.config.MSCConfig.ECCDContractAddress, proofKey, heightHex, this.restClient)
		if err != nil {
...
func (this *MSCManager) commitProof(height uint32, proof []byte, value []byte, txhash []byte) (string, error) {
...
func (this *MSCManager) parserValue(value []byte) []byte {
...
func (this *MSCManager) CheckDeposit() {
	checkTicker := time.NewTicker(config.MSC_MONITOR_INTERVAL)
	for {
		select {
		case <-checkTicker.C:
...
func (this *MSCManager) checkLockDepositEvents() error {

...
func NewPolyManager(servCfg *config.ServiceConfig, startblockHeight uint32, polySdk *sdk.PolySdk, ethereumsdk *ethclient.Client, bridgeSdk *bridgesdk.BridgeSdkPro, boltDB *db.BoltDB) (*PolyManager, error) {
	...
	ks := tools.NewEthKeyStore(servCfg.MSCConfig, chainId)
	accArr := ks.GetAccounts()
	if len(servCfg.MSCConfig.KeyStorePwdSet) == 0 {
		...
		for _, v := range accArr {
			...
			servCfg.MSCConfig.KeyStorePwdSet[strings.ToLower(v.Address.String())] = string(raw)
		}
	}
	if err = ks.UnlockKeys(servCfg.MSCConfig); err != nil {
		return nil, err
	}
	...
	for _, event := range events {
		for _, notify := range event.Notify {
			if notify.ContractAddress == this.config.PolyConfig.EntranceContractAddress {
				...
				if uint64(states[2].(float64)) != this.config.MSCConfig.SideChainId {
					continue
				}
...
func (this *EthSender) commitDepositEventsWithHeader(header *polytypes.Header, param *common2.ToMerkleValue, headerProof string, anchorHeader *polytypes.Header, polyTxHash string, rawAuditPath []byte) bool {
	...
	contractaddr := ethcommon.HexToAddress(this.config.MSCConfig.ECCMContractAddress)
		...
		go func() {
			for v := range c {
				if err = this.sendTxToEth(v); err != nil {
					log.Errorf("failed to send tx to mxc: error: %v, txData: %s", err, hex.EncodeToString(v.txData))
				}
			}
		}()
	}
...
func (this *EthSender) waitTransactionConfirm(polyTxHash string, hash ethcommon.Hash) bool {
	for {
		...
		if ispending == true {
			...
			if !result {
				log.Infof("( eth_transaction %s, poly_tx %s block %v ) receipt.Status: %d", hash.String(), polyTxHash, receipt.BlockNumber, receipt.Status)
			}
			return result
		}
	}

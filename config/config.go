...
const (
	MSC_MONITOR_INTERVAL = time.Second
	ONT_MONITOR_INTERVAL = time.Second

	MSC_USEFUL_BLOCK_NUM     = 3
...
func NewServiceConfig(configFilePath string) *ServiceConfig {
	...
	for k, v := range servConfig.MSCConfig.KeyStorePwdSet {
		delete(servConfig.MSCConfig.KeyStorePwdSet, k)
		servConfig.MSCConfig.KeyStorePwdSet[strings.ToLower(k)] = v
	}
